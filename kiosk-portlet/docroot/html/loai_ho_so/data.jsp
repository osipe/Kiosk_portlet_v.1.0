...
<%
	...
	long linhVucId = ParamUtil.getInteger(request, "linhVucId");
%>
<aui:form name="frmDS" method="post">
	<liferay-portlet:renderURL varImpl="iteratorURL" windowState="<%=LiferayWindowState.EXCLUSIVE.toString()%>">
		<portlet:param name="mvcPath" value="/html/loai_ho_so/data.jsp" />
	...
	<liferay-ui:search-container var="searchContainer" emptyResultsMessage="there-are-no-results" iteratorURL="<%=iteratorURL %>" delta="10" deltaConfigurable="false" >
		<%
			List<LoaiHoSo> items = LoaiHoSoLocalServiceUtil.findBy_C_G_H_T_M_M(0, 0, 
					hoatDong, linhVucId, ten, ma, 
					moTa, true, searchContainer.getStart(), searchContainer.getEnd(), searchContainer.getOrderByComparator());
			int count = LoaiHoSoLocalServiceUtil.countBy_C_G_H_T_M_M(0, 0, hoatDong, linhVucId, ten, ma, moTa, true);
		%>
		...
		<liferay-ui:search-container-row className="com.alpha.portlet.danh_muc.model.LoaiHoSo" escapedModel="<%= true %>" modelVar="item" rowVar="entry" keyProperty="loaiHoSoId">
			<portlet:renderURL var="editURL" windowState="<%=LiferayWindowState.POP_UP.toString()%>">
				<portlet:param name="mvcPath" value="/html/loai_ho_so/add.jsp" />
				<portlet:param name="loaiHoSoId" value="<%=String.valueOf(item.getLoaiHoSoId())%>" />
			</portlet:renderURL>
			<portlet:resourceURL var="hoatDongURL" id="hoatDongURL">
				...
				<%
					...
					String tenHoatDong = item.getHoatDong() ? "Hoạt động" : "Ngưng hoạt động";
			 	%>
			...
			<liferay-ui:search-container-column-text name="Hoạt Động" align="center" value="<%= tenHoatDong%>"/>
			<liferay-ui:search-container-column-text name="Độ ưu tiên" align="center" property="doUuTien"/>
			<liferay-ui:search-container-column-text name="Tên lĩnh vực" align="center" property="tenLinhVuc"/>
