...
public class LoaiHoSoFinderImpl extends BasePersistenceImpl<LoaiHoSo> implements LoaiHoSoFinder {

	public static final String COUNT_BY_C_G_H_T_M_M = LoaiHoSoFinder.class.getName() + ".countBy_C_G_H_T_M_M";
	public static final String FIND_BY_C_G_H_T_M_M = LoaiHoSoFinder.class.getName() + ".findBy_C_G_H_T_M_M";
	...
	public int countBy_C_G_H_T_M_M(long companyId, long groupId, int hoatDong, long linhVucId, String ten, String ma, String moTa, boolean andOperator) throws SystemException {
		...
		return countBy_C_G_H_T_M_M(companyId, groupId, hoatDong, linhVucId, new String[] { ten }, new String[] { ma }, new String[] { moTa }, andOperator);
	}

	public int countBy_C_G_H_T_M_M(long companyId, long groupId, int hoatDong, long linhVucId, String[] tens, String[] mas, String[] moTas, boolean andOperator) throws SystemException {
		...
		try {
			session = openSession();
			String sql = CustomSQLUtil.get(COUNT_BY_C_G_H_T_M_M);
			if (hoatDong == 0) {
			...
			if (linhVucId == 0) {
				sql = StringUtil.replace(sql, "AND linhVucId = ?", StringPool.BLANK);
			}
	...
	public List<LoaiHoSo> findBy_C_G_H_T_M_M(long companyId, long groupId, int hoatDong, long linhVucId, String ten, String ma, String moTa, boolean andOperator, int start, int end, OrderByComparator obc) throws SystemException {
		...
		return findBy_C_G_H_T_M_M(companyId, groupId, hoatDong, linhVucId, new String[] { ten }, new String[] { ma }, new String[] { moTa }, andOperator, start, end, obc);
	}

	public List<LoaiHoSo> findBy_C_G_H_T_M_M(long companyId, long groupId, int hoatDong, long linhVucId, String[] tens, String[] mas, String[] moTas, boolean andOperator, int start, int end, OrderByComparator obc) throws SystemException {
		...
		try {
			session = openSession();
			String sql = CustomSQLUtil.get(FIND_BY_C_G_H_T_M_M);
			...
			SQLQuery q = session.createSQLQuery(sql);
			q.addEntity("LoaiHoSo", LoaiHoSoImpl.class);
			QueryPos qPos = QueryPos.getInstance(q);
			if(groupId > 0){
				qPos.add(groupId);
			}
			...
			qPos.add(mas, 2);
			qPos.add(moTas, 2);
			return (List<LoaiHoSo>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		} finally {
			closeSession(session);
